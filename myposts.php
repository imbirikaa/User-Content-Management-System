...
<body>
  ...
  <div class="posts d-flex justify-content-center">
    ...
    ?>
      <?php
      $sql = "SELECT users.user_First , users.user_Last , posts.post_cont ,posts.post_ID, posts.post_Date FROM users RIGHT JOIN posts ON users.user_ID = posts.post_Creator WHERE users.user_ID = $id ORDER BY posts.post_Date DESC ;";
      ...
      <?php
        ...
        <nav class="navbar rounded-3 justify-content-center title fs-4 p-3 mb-2 bg-danger-subtle text-danger-emphasis">
          -- NO POSTS FOUND -- <br>
          Try to create a Post from The Menu above 
        </nav>
      </div>';
      }
      ?>
